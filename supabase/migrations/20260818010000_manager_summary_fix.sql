...
DECLARE
  ...
  v_offers_made int;
BEGIN
  -- 1. RDV commerciaux réalisés (calendar_events de type commercial selon whitelist)
  ...
  WHERE workspace_id = p_workspace_id
    ...
    AND event_type IN ('rdv_prospection', 'rdv_client_suivi', 'presentation_rt', 'soutenance', 'atelier_client', 'suivi_mission_client');
  ...
  FROM (
    ...
    WHERE ce.workspace_id = p_workspace_id
      ...
      AND ce.event_type IN ('rdv_prospection', 'rdv_client_suivi', 'presentation_rt', 'soutenance', 'atelier_client', 'suivi_mission_client')
