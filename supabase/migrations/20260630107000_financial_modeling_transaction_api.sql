-- Additive migration: financial_modeling_transaction_api
-- Adjust calculation_version type and add transactional RPCs for persistence.

begin;

-- 1. Correct calculation_version type
alter table public.financial_models drop constraint if exists financial_models_calculation_version_check;

alter table public.financial_models alter column calculation_version type text;

alter table public.financial_models add constraint financial_models_calculation_version_check
  check (length(trim(calculation_version)) > 0);

-- 2. Create transactional save RPC
...
begin
  ...
  if v_current_status is null then
    ...
    if v_current_status = 'converted' or v_current_status = 'archived' then
      raise exception 'Le modèle financier est verrouillé (converti ou archivé) et ne peut plus être modifié.' using errcode = 'LOCKED';
    end if;

    if p_expected_updated_at is not null and v_current_updated_at <> p_expected_updated_at then
      raise exception 'Conflit de mise à jour : la simulation a été modifiée entre-temps.' using errcode = 'VERSION_CONFLICT';
    end if;
...
begin
  ...
  if v_status = 'converted' then
    raise exception 'Impossible d''archiver une simulation déjà convertie.' using errcode = 'LOCKED';
  end if;
...
revoke execute on function public.save_financial_model_snapshot(uuid, timestamptz, jsonb, jsonb) from public, anon;
grant execute on function public.save_financial_model_snapshot(uuid, timestamptz, jsonb, jsonb) to authenticated;

revoke execute on function public.archive_financial_model(uuid) from public, anon;
grant execute on function public.archive_financial_model(uuid) to authenticated;
