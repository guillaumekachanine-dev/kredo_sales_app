...
| Chantier | État | Où |
|---|---|---|
| **Account Intelligence — refonte de l'acquisition de connaissance compte** | **Lot 0 + canal LLM externe (0.8) + Lot 1 livrés (2026-09-11).** INTEL-035 fetch réellement ; la recherche approfondie est déléguée à un LLM externe (Deep Research), importée via `(dev)/account-knowledge-import` ; le **renderer V4** existe (`account-knowledge-v4/`, bandeau d'ancrage, badges épistémiques, modes de lecture, sources par affirmation, lecteur « Rapport complet » bâti sur `content_json` — `content_text` inutile). **Prochain : structuration répétable** rapport Deep Research → JSON V4 (faite à la main deux fois) | `docs/FEATURES/cockpit_intelligence_features/account_intelligence/` — **commencer par le `README.md`**, point de reprise `11` |
