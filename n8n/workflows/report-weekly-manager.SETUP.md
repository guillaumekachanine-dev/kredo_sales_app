# ADR-0010 Lot 2 — report-weekly-manager : import & configuration VPS
...
depuis n8n. **`report-weekly-manager` n'a pas ce nœud.**

Le brief hebdomadaire a besoin de `loadAgendaSnapshot()` — l'agrégateur agenda de Kredo
...
src/app/api/reports/weekly-manager/trigger/route.ts
  ...
  → POST /webhook/report-weekly-manager avec input.facts déjà complet
```
...
1. n8n → **Workflows → Import from File** → sélectionner `report-weekly-manager.json`.
...
2. Depuis Kredo (dev), appeler `POST /api/reports/weekly-manager/trigger` (body vide = semaine ISO en
   ...
   `{N8N_WEBHOOK_BASE_URL}/webhook/report-weekly-manager`.
...
4. Vérifier dans Supabase que `ai_intelligence_runs.status` passe à `succeeded` et qu'une ligne apparaît
   dans `ai_intelligence_results` avec `phase = 1`, `result_type = 'weekly_manager'`.
