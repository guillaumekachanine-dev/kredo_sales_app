  ...
  return (
    <section aria-labelledby="competitive-mobile-matrix-title" className="border-y border-edito-border bg-edito-surface px-4 py-4">
        ...
        {positionedActors.length > 0 ? (
          <svg viewBox={`0 0 ${width} ${height}`} className="block h-[230px] w-full touch-pan-y" role="img" aria-label="Mini-matrice tactile des acteurs concurrents">
            {[0, 10, 20, 30, 35].map((tick) => (
              <g key={`x-${tick}`}>
                <line x1={x(tick)} y1={margin.top} x2={x(tick)} y2={margin.top + plotHeight} stroke="var(--color-edito-border)" strokeWidth={0.8} />
                <text x={x(tick)} y={height - 17} textAnchor="middle" fontSize={9} fill="var(--color-edito-muted)">{tick}</text>
              </g>
            ))}
            {[1, 3, 5].map((tick) => (
              <g key={`y-${tick}`}>
                <line x1={margin.left} y1={y(tick)} x2={margin.left + plotWidth} y2={y(tick)} stroke="var(--color-edito-border)" strokeWidth={0.8} />
                <text x={margin.left - 12} y={y(tick) + 3} textAnchor="middle" fontSize={9} fill="var(--color-edito-muted)">{tick}</text>
              </g>
            ))}
            <text x={margin.left + plotWidth / 2} y={height - 4} textAnchor="middle" fontSize={9} fontWeight={700} fill="var(--color-edito-body)">Appétence</text>
            ...
            {positionedActors.map((actor) => {
              ...
              return (
                ...
                >
                  ...
                  <circle cx={cx} cy={cy} r={actor.isBenchmarkAccount ? 7 : 6} fill={CATEGORY_COLORS[actor.category]} stroke={actor.isBenchmarkAccount ? "var(--color-edito-ink)" : "var(--color-edito-surface)"} strokeWidth={actor.isBenchmarkAccount ? 2.5 : 1} />
                  {showLabel ? (
                    <text x={labelOnLeft ? cx - 12 : cx + 12} y={cy + 3} textAnchor={labelOnLeft ? "end" : "start"} fontSize={9} fontWeight={700} fill="var(--color-edito-ink)">
          ...
          <div className="flex h-[180px] items-center justify-center px-8 text-center text-xs leading-relaxed text-edito-muted">
      ...
      {nonPositionedActors.length > 0 ? (
        <div className="mt-3">
          <p className="text-[10px] font-bold uppercase tracking-[0.09em] text-edito-muted">Non positionnés</p>
          <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
            {nonPositionedActors.map((actor) => (
              <button key={actor.id} type="button" aria-pressed={actor.id === selectedActorId} onClick={() => onSelectActor(actor.id)} className="min-h-11 shrink-0 rounded-full border border-edito-border px-3 text-xs font-semibold text-edito-body hover:border-edito-navy hover:text-edito-navy focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-brass aria-pressed:border-brand-brass aria-pressed:bg-brand-brass/10 aria-pressed:text-brand-brass">
                {actor.name}
              </button>
            ))}
          </div>
