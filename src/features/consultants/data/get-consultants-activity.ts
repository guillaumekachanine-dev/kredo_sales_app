...
import type {
  ...
  YtdActivityRow,
} from "@/features/consultants/activity/activity.types"

// ─────────────────────────────────────────────────────────────────────────────
//  Loader du chapitre « Activités & congés » (Lot 5).
//
//  Extrait à l'identique de `src/app/(app)/consultants/(tabbed)/activite-conges/
//  page.tsx` — 6 lectures parallèles (3 vues analytiques + 3 tables). Chaque
//  lecture est isolée : une source en échec dégrade l'écran, ne le casse pas
//  (`sourceIssues`). Aucun recalcul métier : tout vit dans `ActivityDashboard`.
//  RLS de l'utilisateur — `collaborator_compensation` (owner/admin) peut
//  revenir vide, le composant affiche alors « Acces admin requis ».
// ─────────────────────────────────────────────────────────────────────────────
...
export async function getConsultantsActivity(): Promise<ActivityDashboardData> {
  ...
  const [summaries, ytd, alerts, absences, closures, compensations] = await Promise.all([
