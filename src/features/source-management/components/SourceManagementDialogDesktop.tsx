...
export function SourceManagementDialogDesktop({
  ...
  const activeCorpus = view.kind === "corpus"
    ? [...snapshot.sectorCorpora, ...snapshot.thematicCorpora, ...snapshot.accountCorpora].find((c) => c.id === view.corpusId)
    : null
  ...
  const leftPaneNav = (
    <div className="p-4 space-y-2">
      ...
      {/* 5. Corpus comptes (Account Intelligence) */}
      <div className="pt-2 space-y-1 border-t border-white/5">
        <p className="px-3 pb-1 text-[10px] font-bold uppercase tracking-wider text-white/40">Corpus comptes</p>
        {snapshot.accountCorpora.length === 0 ? (
          <p className="px-3 py-1 text-[10px] text-white/40 italic">Aucun corpus de compte importé</p>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-[10px] text-white/50 truncate">
                    {corpus.totalSources} sources · {corpus.activationState === "active" ? "Actif" : "Brouillon"}
                  </p>
                </div>
                <ChevronRight
                  className={cn(
