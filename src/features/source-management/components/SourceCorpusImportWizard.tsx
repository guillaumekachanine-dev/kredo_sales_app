...
import { Switch } from "@/components/intelligence/WatchSettingsDialogShell"
import {
...
export function SourceCorpusImportWizard({ variant, onClose }: SourceCorpusImportWizardProps) {
  ...
  return (
    <div className="space-y-4">
      {/* ── STEP HEADER BAR ───────────────────────────────────────── */}
      <div className="rounded-lg border border-edito-border bg-edito-navy p-2.5 text-white shadow-xs">
      ...
      {/* ── STEP 1: PRÉPARER (Compacifiée) ────────────────────────── */}
      {step === "prepare" && (
        <div className="space-y-3.5">
          <div className={cn("grid gap-3", isMobile ? "grid-cols-1" : "grid-cols-2")}>
            <label className="group relative flex flex-col items-center justify-center cursor-pointer rounded-lg border border-dashed border-edito-border bg-white px-4 py-3.5 text-center shadow-xs transition-all hover:border-edito-brass hover:bg-edito-chip/50">
              <span className="flex size-7 items-center justify-center rounded-md bg-edito-chip text-xs font-black text-edito-navy group-hover:bg-edito-brass group-hover:text-edito-navy transition-colors" aria-hidden="true">
                ↑
              </span>
              <span className="mt-1.5 block text-xs font-bold text-edito-navy">Déposer le JSON du registre E3</span>
              <span className="mt-0.5 block text-[10px] font-medium text-edito-muted">{fileName ?? "ou parcourir les fichiers"}</span>
              <input
                type="file"
            ...
            <div className="flex flex-col">
              <label className="mb-1 text-[10px] font-black uppercase tracking-wider text-edito-navy" htmlFor="source-corpus-json">
              ...
              <textarea
                ...
                className="w-full flex-1 resize-y rounded-lg border border-edito-border bg-white p-2 font-mono text-[10px] leading-relaxed text-edito-navy outline-none shadow-xs focus:border-edito-brass focus:ring-1 focus:ring-edito-brass/20"
              />
            </div>
          </div>

          {/* Signature Action Bar */}
          <div className="rounded-lg border border-edito-border bg-edito-navy p-2.5 flex flex-wrap items-center justify-between gap-2 text-white shadow-xs">
            <div className="flex items-center gap-2">
              <span className="size-2 rounded-sm bg-edito-brass animate-pulse" aria-hidden="true" />
          ...
          {parseErrors.length > 0 && (
            <ul className="space-y-1 rounded-lg border border-rose-300 bg-rose-50 p-3">
              {parseErrors.map((message, index) => (
                <li key={index} className="text-[10px] font-medium leading-relaxed text-rose-800">{message}</li>
              ))}
            </ul>
          )}

          {resolveError && (
            <p className="rounded-lg border border-rose-300 bg-rose-50 p-3 text-[10px] font-medium text-rose-800">{resolveError}</p>
          )}

          {parsed && (
            <div className="rounded-lg border border-edito-border bg-white p-3.5 shadow-xs space-y-3">
              <div className="flex items-center justify-between border-b border-edito-chip pb-2">
                <h4 className="text-[10px] font-black uppercase tracking-wider text-edito-navy">Synthèse de l&apos;analyse E3</h4>
              ...
              <div className={cn("grid gap-2 text-[10px]", isMobile ? "grid-cols-1" : "grid-cols-3")}>
                <div className="rounded-md border border-edito-border bg-edito-canvas p-2">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-edito-muted">Segment / Secteur</p>
                  <p className="mt-0.5 font-bold text-edito-navy">{parsed.meta.segmentSlug}</p>
                  <p className="text-[9px] text-edito-muted truncate">{parsed.meta.secteur ?? "—"}</p>
                </div>
                <div className="rounded-md border border-edito-border bg-edito-canvas p-2">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-edito-muted">Snapshot / Version</p>
                  <p className="mt-0.5 font-bold text-edito-navy">v{parsed.meta.version}</p>
                  <p className="text-[9px] text-edito-muted">{parsed.meta.dateSnapshot}</p>
                </div>
                <div className="rounded-md border border-edito-border bg-edito-canvas p-2">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-edito-muted">Volumétrie Sources</p>
                  <p className="mt-0.5 font-bold text-edito-navy">{parsed.sources.length} sources</p>
                  <p className="text-[9px] text-edito-muted">{parsed.collectableCount} collectables · {parsed.staticCount} statiques</p>
                </div>
              </div>

              <div className="flex flex-wrap items-center justify-between gap-2 border-t border-edito-chip pt-2 text-[10px] text-edito-muted">
              ...
              <div className="flex justify-end border-t border-edito-chip pt-2">
      ...
      {/* ── STEP 2: ARBITRER (Tableau épuré à 4 colonnes avec Switch) ─── */}
      {step === "arbitrate" && (
        <div className="space-y-3.5">
          <div className="flex items-center justify-between rounded-md border border-edito-border bg-white px-3.5 py-2 text-[11px] font-medium text-edito-navy shadow-xs">
            <span>
          ...
          <div className={cn(isMobile ? "space-y-2" : "overflow-hidden rounded-lg border border-edito-border bg-white shadow-xs")}>
            {isMobile ? (
          ...
          <div className="flex items-center justify-between gap-2 border-t border-edito-border pt-3">
      ...
      {step === "finalize" && parsed && !result && (
        <div className="space-y-4">
          <div className="rounded-lg border border-edito-border bg-white p-4 shadow-xs space-y-3">
            <div className="flex items-center justify-between border-b border-edito-chip pb-2">
            ...
            <div className={cn("grid gap-2.5 text-[11px]", isMobile ? "grid-cols-1" : "grid-cols-2")}>
              <div className="rounded-md border border-edito-border bg-edito-canvas p-2.5 space-y-1">
            ...
            <div className="rounded-md border border-edito-border bg-edito-chip/50 p-2.5 text-[11px] text-edito-navy flex flex-wrap justify-between gap-2">
          ...
          {submitError && (
            <p className="rounded-lg border border-rose-300 bg-rose-50 p-3 text-[11px] font-medium text-rose-800">{submitError}</p>
          )}
      ...
      {result && !result.error && (
        <div className="rounded-lg border border-emerald-300 bg-emerald-50 p-5 space-y-3 text-center">
...
function ArbitrationRow({ item, decision, onToggle }: { item: SourceCorpusItemPreview; decision: ItemDecision | undefined; onToggle: (srcId: string, enabled: boolean) => void }) {
  ...
  return (
    <tr className="hover:bg-edito-chip/40 transition-colors align-middle">
      <td className="px-3.5 py-2">
        <p className="font-bold text-edito-navy text-xs">{item.mappedPublisher ?? item.srcId}</p>
      ...
      <td className="px-3.5 py-2 text-edito-navy text-[11px]">
        {item.input.contentTemporality}
        <span className="ml-2 text-[10px] text-edito-muted">({item.input.usageScopes.join(", ") || "—"})</span>
      </td>
      <td className="px-3.5 py-2 text-right">
        {item.isCollectable ? (
          <div className="inline-flex items-center gap-2">
            <span className="text-[10px] font-bold text-edito-navy">{enabled ? "Actif" : "Exclu"}</span>
            <Switch label={item.mappedPublisher ?? item.srcId} checked={enabled} onChange={(checked) => onToggle(item.srcId, checked)} />
          </div>
        ) : (
...
function ArbitrationCard({ item, decision, onToggle }: { item: SourceCorpusItemPreview; decision: ItemDecision | undefined; onToggle: (srcId: string, enabled: boolean) => void }) {
  const enabled = decision?.isEnabled ?? item.isEnabledDefault
  return (
    <div className="rounded-lg border border-edito-border bg-white p-3 shadow-xs space-y-2">
