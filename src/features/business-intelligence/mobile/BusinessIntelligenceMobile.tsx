...
export function BusinessIntelligenceMobile({ viewModel, snapshot, sectorMapCatalog, competitiveMapWorkspace, initialSection = "priorities" }: { viewModel: BusinessIntelligenceMobileViewModel; snapshot: BusinessIntelligenceSnapshot; sectorMapCatalog: SectorMapCatalog; competitiveMapWorkspace: CompetitiveMapWorkspace; initialSection?: MobileSection }) {
  ...
  return <main className="min-h-dvh overflow-x-hidden bg-canvas pb-[max(1rem,env(safe-area-inset-bottom))] text-white">
    <BusinessIntelligenceMobileHeader period={period} onPeriodChange={setPeriod} showPeriodPicker={section !== "competitive_env"} />
    <nav aria-label="Sections Business Intelligence" className="sticky top-0 z-20 border-b border-white/10 bg-canvas/95 px-4 py-2 backdrop-blur-sm"><div className="grid grid-cols-5 gap-1 rounded-xl bg-white/[0.035] p-1" role="tablist">{([['priorities', 'Priorités'], ['windows', 'Fenêtres'], ['sectors', 'Secteurs'], ['value_chain', 'Chaîne'], ['competitive_env', 'Concurrents']] as const).map(([id, label]) => { const selected = section === id; return <button key={id} id={`bi-mobile-tab-${id}`} type="button" role="tab" aria-selected={selected} aria-controls="bi-mobile-panel" onClick={() => setSection(id)} className={`min-h-11 rounded-lg px-0.5 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-brass ${selected ? "bg-white/10 text-white underline decoration-brand-brass decoration-2 underline-offset-4" : "text-white/55"}`}>{label}</button> })}</div></nav>
