        ...
        {accounts.map((account) => {
          ...
          return (
            <button key={account.accountId} type="button" onClick={() => onSelectAccount(account.accountId)} aria-pressed={selected} className={`w-full rounded-xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-brass motion-reduce:transition-none ${selected ? "border-brand-brass/50 bg-brand-brass/[0.08]" : "border-white/10 bg-white/[0.025] hover:bg-white/[0.05]"}`}>
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{account.name}</p><p className="mt-1 text-[11px] text-white/50">{account.sectorName ?? "Secteur non renseigné"}</p></div><span className="shrink-0 text-[10px] font-semibold text-white/55">{account.openOpportunityCount} opportunité(s)</span></div>
              <div className="mt-3 grid grid-cols-3 gap-2"><IndicatorBar label="Reach" value={account.reach} /><IndicatorBar label="Momentum" value={account.momentum} /><IndicatorBar label="Inactivité" value={account.inactivityRisk} /></div>
