...
export type CommercialActivityFilterNature = CommercialActivityNature | "commercial"
export type CommercialActivityCategory = CommercialActivityNature | "unclassified"
...
export type CommercialActivitySnapshot = {
  ...
  timeline: Array<{
    ...
    completedHours: number
    plannedHours: number
    byNature: Partial<Record<CommercialActivityNature, { count: number; hours: number }>>
  }>
  distribution: Array<{
  ...
  outcomes: {
    ...
    needsCreated: number
    proposalsOrDefenses: number
    cvsSent: number
    candidateOrClientInterviews: number
