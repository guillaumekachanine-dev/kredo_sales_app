...
// ─── Import d'une étude ChatGPT Deep Research (Desktop) ─────────────────────
// Parcours : PDF → extraction (texte intégral, contrôle d'intégrité) → conversion
// (passes du modèle, suivies ici) → rapport de couverture → publication sur le compte
// et téléchargement des deux fichiers. Chaque étape affiche ce qu'elle a produit :
...
const PRIMARY_BUTTON =
  "inline-flex min-h-9 items-center justify-center rounded border border-edito-navy bg-edito-navy px-4 text-xs font-bold text-edito-surface transition-colors hover:bg-edito-heading disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-edito-brass/60"
...
export function AccountStudyImportDialog({
  ...
  // Suivi : relance périodique tant que la conversion tourne. S'arrête d'elle-même.
  useEffect(() => {
    if (!open || step !== "tracking" || !studyId) return
    let cancelled = false
    const tick = async () => {
  ...
  }, [open, step, studyId, refreshProgress, router])
  ...
  return (
    <AppDialog
      ...
      title={
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-edito-muted">Étude de l&apos;entreprise — {companyName}</p>
          <h2 className="mt-0.5 font-heading text-base font-bold text-edito-navy">Importer une étude ChatGPT Deep Research</h2>
        </div>
      }
      className="bg-edito-surface sm:!max-w-2xl"
      ...
      <div className="space-y-4 pb-2">
        ...
        {step === "select" || step === "extracting" ? (
          <div className="space-y-3">
            ...
            <div className="flex justify-end">
              <button type="button" onClick={extract} disabled={!file || step === "extracting"} className={PRIMARY_BUTTON}>
        ...
        {step === "extracted" && intake ? (
          <div className="space-y-3">
            ...
            <div className="flex justify-end">
              <button type="button" onClick={convert} disabled={pending} className={PRIMARY_BUTTON}>
        ...
        {step === "tracking" ? (
          <div className="space-y-3">
            ...
            {status === "ready" ? (
              <div className="space-y-3">
                {progress?.coverage ? <StudyCoverageReport coverage={progress.coverage} /> : null}
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => download("knowledge")} disabled={pending} className={SECONDARY_BUTTON}>
                      Briques (JSON)
                    </button>
                    <button type="button" onClick={() => download("sources")} disabled={pending} className={SECONDARY_BUTTON}>
                      Sources E3 (JSON corpus)
                    </button>
                    <button type="button" onClick={() => download("raw")} disabled={pending} className={SECONDARY_BUTTON}>
                      Texte intégral (.md)
                    </button>
                  </div>
                  {progress?.publishedAt ? (
                    ...
                    <button type="button" onClick={publish} disabled={pending} className={PRIMARY_BUTTON}>
