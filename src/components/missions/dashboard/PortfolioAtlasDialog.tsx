...
export function PortfolioAtlasDialog({ open, onOpenChange, overview }: PortfolioAtlasDialogProps) {
  ...
  return (
    <AppDialog
      ...
      title={
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.18em] text-primary">Atlas du portefeuille</p>
          <h2 className="font-heading text-lg font-black text-heading">Engagements réalisés · {overview.year}</h2>
        </div>
      }
      description="Une lecture à la fois, alimentée uniquement par les missions AT et projets actifs."
      className="!h-[min(92dvh,820px)] !w-[min(calc(100vw-1rem),1280px)] !max-w-none"
      maxHeightClassName="max-h-[min(92dvh,820px)]"
