  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="mb-3 flex shrink-0 flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2 text-[10px] font-bold"><span className="rounded-full bg-primary/10 px-3 py-1 text-primary">Premier client : {firstClientPct.toFixed(1)}%</span><span className="rounded-full bg-heading px-3 py-1 text-white">Top 3 : {top3ClientsPct.toFixed(1)}%</span></div>
      </header>

      <div className="hidden min-h-0 flex-1 grid-cols-[minmax(0,1fr)_240px] gap-3 md:grid">
        <div className="relative min-h-[390px] overflow-hidden rounded-[var(--radius-medium)] border border-border bg-canvas">
          {rects.map(({ x, y, width, height, item }) => {
            ...
            return (
              <button key={item.companyId} type="button" onClick={() => setSelectedId(item.companyId)} aria-pressed={selectedId === item.companyId} className="absolute overflow-hidden border border-surface/80 p-2 text-left text-white outline-none transition-[filter] hover:brightness-95 focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-heading motion-reduce:transition-none" style={{ left: `${x}%`, top: `${y}%`, width: `${width}%`, height: `${height}%`, background }}>
      ...
      <div className="min-h-0 flex-1 md:hidden">
        <div className="mb-3 grid grid-cols-2 rounded-[var(--radius-medium)] border border-border bg-canvas p-1" role="group" aria-label="Vue mobile de l’exposition">
          {(["clients", "engagements"] as const).map((value) => <button key={value} type="button" aria-pressed={mobileMode === value} onClick={() => setMobileMode(value)} className={cn("min-h-11 rounded-[var(--radius-small)] text-xs font-bold", mobileMode === value ? "bg-primary text-primary-fg" : "text-body")}>{value === "clients" ? "Clients" : "Engagements"}</button>)}
        </div>
        {mobileMode === "clients" ? <ol className="space-y-2">{clients.map((client) => <li key={client.companyId}><button type="button" onClick={() => setSelectedId(selectedId === client.companyId ? null : client.companyId)} className="min-h-14 w-full rounded-[var(--radius-small)] border border-border p-2 text-left"><span className="flex justify-between gap-3 text-[11px] font-bold text-heading"><span className="truncate">{client.companyName}</span><span>{formatEuroCompact(client.revenue)}</span></span><span className="mt-1 block h-2 rounded-full bg-border"><span className="block h-full rounded-full bg-primary" style={{ width: `${Math.max(2, (client.revenue / maxRevenue) * 100)}%` }} /></span>{selectedId === client.companyId && <span className="mt-2 block text-[9px] text-body">{client.engagements.map((item) => item.title).join(" · ")}</span>}</button></li>)}</ol> : <div className="space-y-4">{engagementGroups.map(([label, points]) => <section key={label}><h4 className="mb-2 text-[10px] font-black uppercase tracking-wider text-heading">{label}</h4><ul className="space-y-2">{points.map((point) => <li key={`${point.type}-${point.id}`} className="rounded-[var(--radius-small)] border border-border p-3"><div className="flex justify-between gap-3"><div><p className="text-[11px] font-bold text-heading">{point.companyName}</p><p className="text-[10px] text-body">{point.title}</p></div><span className={cn("mt-1 size-3 shrink-0", point.type === "mission" ? "rounded-full bg-primary" : "rotate-45 rounded-sm bg-[var(--color-dataviz-2)]")} /></div><div className="mt-2 grid grid-cols-3 text-[9px]"><span><b className="block text-heading">{formatEuroCompact(point.revenueYtd)}</b>CA</span><span><b className="block text-heading">{formatPct(point.actualMarginPct)}</b>Marge</span><span><b className="block text-heading">{point.daysUntilEnd === null ? "—" : `${point.daysUntilEnd} j`}</b>Échéance</span></div></li>)}</ul></section>)}</div>}
      </div>
    </div>
  )
}
