...
export function MissionPlanningTab({ vm }: MissionPlanningTabProps) {
  ...
  const eventInitialValues: AgendaEventDrawerInitialValues = {
    title: `Suivi mission · ${mission.title}`,
    event_type: "rdv_client_suivi",
    company: vm.company ? { id: vm.company.id, name: vm.company.name, isNew: false } : null,
    mission_id: mission.id,
    collaborator_id: vm.collaborator?.id || undefined,
    contact_id: vm.contacts[0]?.id || undefined,
  }

  const PlanifierAction = (
    <div className="flex items-center justify-end">
      ...
      >
        <span className="text-primary font-bold mr-0.5">+</span> Planifier
      </Button>
    </div>
  )
