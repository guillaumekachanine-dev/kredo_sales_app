...
interface UnifiedPlanningViewProps {
  opportunities: OpportunityPlanningData[]
  staffingData: StaffingPlanningData[]
...
const STAFFING_MILESTONE_COLORS: Record<string, string> = {
...
// ─── CALCUL DES JALONS D'OPPORTUNITÉ ──────────────────────────────────────────
...
function getOpportunityMilestones(opp: OpportunityPlanningData, today: Date): Milestone[] {
  ...
  addMilestone(
    ...
    "lightbulb",
    "Publication ou détection de l'opportunité."
  )

  let qualDate = new Date(openedDate.getTime() + 2 * 24 * 60 * 60 * 1000)
  const qualInt = opp.interactions.find(i => i.type === "qualification" || i.summary?.toLowerCase().includes("qualification"))
  if (qualInt) qualDate = new Date(qualInt.occurredAt)
  addMilestone(
    ...
    "chat",
    qualInt?.summary || "Échange téléphonique ou réunion de qualification."
  )

  let pushDate = new Date(openedDate.getTime() + 6 * 24 * 60 * 60 * 1000)
  const firstCand = opp.candidates.find(c => c.sentToClientAt)
  if (firstCand && firstCand.sentToClientAt) pushDate = new Date(firstCand.sentToClientAt)
  if (opp.candidates.length > 0) {
    addMilestone(
      ...
      "document-user",
      opp.candidates.map(c => c.fullName).join(", ")
    )
  }
...
export function UnifiedPlanningView({
  opportunities,
  staffingData,
  scale = "year",
}: UnifiedPlanningViewProps) {
  ...
  const mappedRows = useMemo(() => {
    // 1. Transformer les besoins
    const needRows = opportunities.map((opp) => {
      const allMilestones = getOpportunityMilestones(opp, today)
      const milestones = allMilestones.filter((m) => m.date >= range.start && m.date <= range.end)
      ...
      return {
        ...
        client: opp.client,
        staffingCount: opp.candidates.length,
        milestones,
        firstOffset,
        lastOffset,
      }
    ...
    // 2. Transformer les staffings (candidats)
    const staffingRows = staffingData.map((staff) => {
      const milestones = staff.milestones.map((m) => {
        const iconMap: Record<string, string> = {
          identification: "lightbulb",
          cv_sent: "document-user",
          prequal: "chat",
          client_interview: "chat",
          decision: "pencil",
          demarrage: "flag",
        }
        return {
          ...
          type: m.type,
          label: m.label,
          color: STAFFING_MILESTONE_COLORS[m.type] || "bg-muted border-border text-white",
          iconName: iconMap[m.type] || "user-check",
          description: m.description || "",
          status: m.isFuture ? ("planned" as const) : ("completed" as const),
        }
      ...
      return {
        ...
        subtitle: staff.opportunityTitle || "",
        logoPath: null,
        website: null,
        client: "",
        staffingCount: 0,
    ...
    // Fusionner
    return [...needRows, ...staffingRows]
  }, [opportunities, staffingData, range, today])
  ...
  return (
    <div className="relative flex flex-col gap-5 select-none">
      <EntityPlanningView
        ...
        emptyState={
          <div>
            ...
            <p className="mt-1 text-xs text-muted">Aucun jalon sur cette période.</p>
          </div>
        }
        renderRowLabel={(row) => (
          <div className="flex min-w-0 items-center gap-3">
            {/* Moyen visuel discret et élégant de distinguer : liseré coloré vertical arrondi à gauche */}
            <div 
              className={cn(
                "w-[3px] h-7 shrink-0 rounded-full",
                row.type === "besoin" ? "bg-[#FFC107]" : "bg-[#9C27B0]"
              )}
              title={row.type === "besoin" ? "Besoin" : "Candidat"}
            />
            ...
            {row.type === "besoin" ? (
              ...
              <div className="size-6 rounded-full bg-[#9C27B0]/10 flex items-center justify-center text-[10px] font-bold text-[#9C27B0] shrink-0 border border-[#9C27B0]/20">
                {row.label.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()}
              </div>
            )}
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-center">
              <button
                ...
                onClick={() => {
                  if (row.type === "besoin") {
                    openOpportunityDrawer(row.id, "besoin")
                  } else {
                    openStaffingDrawer(row.id)
                  }
                }}
                className="block max-w-full truncate text-left text-[11px] font-bold text-heading transition-colors hover:text-primary hover:underline"
              >
                {row.label}
              </button>
              <div className="mt-0.5 flex items-center gap-1.5 truncate text-[9px] text-muted">
                <span className="truncate font-semibold text-heading/70">{row.subtitle}</span>
                {row.type === "besoin" && (
                  <>
                    <span>•</span>
                    <span className="shrink-0 font-semibold text-primary">
                      {row.staffingCount} staffing{row.staffingCount > 1 ? "s" : ""}
                    </span>
                  </>
                )}
              </div>
          ...
          <>
            ...
            {row.milestones.length > 1 ? (
              <div
                ...
                style={{
                  left: `calc(${row.firstOffset}% + 6px)`,
                  width: `calc(${row.lastOffset - row.firstOffset}% - 12px)`,
                }}
              />
            ) : null}
                ...
                {row.milestones.map((milestone) => {
                  ...
                  return (
                    <div
                      key={milestone.key}
                      onMouseEnter={(event) =>
                        handleMilestoneMouseEnter(milestone, row.label, row.subtitle, event)
                      }
                      onMouseLeave={() => setHoveredMilestone(null)}
                      className={cn(
                        "absolute top-1/2 z-20 flex h-5 w-5 -translate-x-1/2 -translate-y-1/2 cursor-help items-center justify-center rounded-full border shadow-sm transition-transform duration-150 hover:scale-125",
                        milestone.color,
                      )}
                      style={{ left: `${offset}%` }}
                    >
      ...
      <div className="rounded-[var(--radius-medium)] border border-border bg-surface px-5 py-4 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3 border-b border-border/60 pb-3">
          <h4 className="text-xs font-bold text-heading uppercase tracking-wider">Légende des catégories & jalons</h4>
          <div className="flex items-center gap-4 text-xs font-semibold">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-[#FFC107]" />
              <span className="text-heading">Besoins</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-[#9C27B0]" />
              <span className="text-heading">Candidats</span>
            </div>
          </div>
        </div>
