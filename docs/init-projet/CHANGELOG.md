...
### 2026-06-10
- **Base de données (Lot 1)** : migration `006_ai_intelligence` appliquée — 3 tables (`ai_intelligence_runs/results/logs`), 2 enums, 10 policies RLS, 17 index dont GIN JSONB + partiel erreurs, 2 triggers `updated_at`, vue `v_ai_intelligence_summary` security_invoker, contrainte `UNIQUE(run_id,phase)`. Types TypeScript régénérés dans `src/types/database.generated.ts`, façade stable `src/types/database.ts`.
...
- **Types TypeScript** générés depuis le schéma live → `src/types/database.generated.ts`, façade stable `src/types/database.ts`, types métier dans `src/types/domain/*`.
