...
- **Shell Mobile unifié sur `/missions`** : la branche Mobile de `page.tsx` ne rendait que la Synthèse (`MissionsDashboardSection`, désormais supprimé avec `EngagementsOverviewSkeleton` — orphelins). Elle lit maintenant `?vue=` et rend `EngagementsMobileShell` (`src/components/missions/engagements/mobile/`), qui distribue `synthese` / `missions-at` / `projets` et charge **uniquement** les données de la vue rendue. Vues `activite-conges` / `planning-at` non exposées sur Mobile (elles ne l'étaient pas avant).
- **Routes `(tabbed)` neutralisées sur Mobile** : `/missions/actives` et `/missions/projets` font `redirect()` vers `/missions?vue=missions-at` / `?vue=projets` quand `device === "mobile"`. Desktop inchangé. `getMobileTabsForPath("/missions")` renvoie désormais 3 onglets `?vue=` ; `MobileSectionRail` sait résoudre l'onglet actif sur un pathname partagé + query `vue` (rétrocompatible : logique query-aware seulement si un onglet porte un `?`).
- **Synthèse** : `EngagementsOverviewMobile` réécrit dans le langage `/reports` (header compact, nav `grid-cols-3` + filet `brand-brass`, surfaces ouvertes, séparateurs fins, plus de gros container arrondi). Données et modules métier (`RevenueOverviewChart`, `RevenueBreakdownChart`, `ActivityWatchModule`, `EngagementRunway`) inchangés — portage de présentation.
- **Missions AT** (`MissionsAtMobileView`) : header + 3 KPI en ligne (`divide-x` : Missions en cours / TJM moyen / Taux de marge moyen — formules reprises de `MissionsActivesContent`) + recherche instantanée client-side (titre / client / collaborateur) + modale Filtres (Client, Practice, Échéances proche/intermédiaire/plus tard-sans, compteur + Réinitialiser). Liste plate façon `DocumentCard` (pattern, pas d'import) : logo carré `CompanyLogo`, titre 2 lignes, collaborateur + échéance, chevron, `min-h-[88px]`, `border-b`, ligne entière cliquable.
- **Barre d'entités** (`MissionsMobileEntityTabs`) : clone de `CrmMobileAccountTabs` sur `useMissionsTabStore` (= store engagements ; **aucun nouveau store**). « ← Liste » = `setActiveTab("home")` sans fermer les onglets, actif souligné `brand-brass`, croix = `closeAllTabs`, sticky. Transition liste → détail droite→gauche via deux keyframes CSS ajoutées à `globals.css` (`kredo-engagements-detail-in` / `-list-in`, `prefers-reduced-motion` respecté).
- **Fiche mission Mobile** (`MissionDetailMobile` refondu) : header = logo + titre + client seulement (plus de `Mission · ref`, StatusPill, pastilles Fin proche / risque, date « Depuis », KPI TJM/marge/activité). Édition du risque relogée en tête de l'onglet Synthèse (dialogue `updateMissionRisk` conservé). 3 onglets Mobile via **`MissionMobileTabId` / `MISSION_MOBILE_TABS`** dédiés (`MISSION_DETAIL_TABS` Desktop 5 onglets intact) : Synthèse = `MissionSynthesisTab` + `MissionFinancialTab` ; Collaborateur = `MissionCollaboratorTab` + `MissionActivityTab` (activité + CRA) ; Planning inchangé. `MissionDetailTabs` réécrit (langage `/reports`) — consommé uniquement par le Mobile.
- **Data** : `getCurrentEngagementMissions()` enrichi de `collaboratorName` via `missions → collaborators → persons` (pattern `get-missions-list`), sans requête par mission, sans lire `companies.metadata` (toujours `meta_logo_path`). Aucune migration, aucun n8n.
- Validation : `typecheck` ✓ · `test` 2410/2410 ✓ (2 fixtures + 1 assertion nav mises à jour) · `check:server-boundary` ✓ · `lint` (fichiers touchés) ✓ · `next build` ✓. Smoke test Mobile viewport iPhone 14 = Guillaume.
...
- Configuration Serper sans VPS : `V4 Serper Discovery` lit prioritairement la variable n8n administrable `SERPER_API_KEY` (`Settings → Variables`), avec `$env.SERPER_API_KEY` seulement en repli. La clé reste absente de l'export Git.
