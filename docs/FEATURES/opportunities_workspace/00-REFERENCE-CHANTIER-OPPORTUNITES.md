...
| Module | Rendu dans le rail à partir de | Avant : |
|---|---|---|
| Matching profil | Lot 10 | non déclaré |
| Simulation devis | Lot 10 | non déclaré (l'action par-ligne existante n'est pas retirée) |
| Post-Mortem | Lot 10 | non déclaré |
| Modélisation de CA | — (jamais dans le périmètre courant) | non déclaré |

Tant qu'aucun module n'est livré, **`contextualModules: undefined`** (Lots 1→9).
...
> Décisions à ajouter pendant l'audit des lots : **OPP-24+** (ex. forme du détail besoin,
...
| **PRODUCT-05** | Relation entre **opportunité commerciale**, **projet avant-vente** et **mission / projet gagné** (`projects` en base) ? | **7 / futur** | **ouverte** — bloque le contenu métier du chapitre Avant-vente ; structure livrable sans y répondre |
...
| ID | Question | Lot cible | Statut |
|---|---|---|---|
| **CROSS-01** | Quel **point d'entrée partagé** expose proprement « Matching profil » (contexte `opportunityId` optionnel) sans dupliquer le composant Consultants ni le moteur ? Extraire un launcher commun ou monter `MatchingDialog` existant ? | **10** | **ouverte** |
| **CROSS-02** | Quel **launcher officiel** ouvre `post-mortem-commercial` depuis un module contextuel (`MissionComposerDesktop` en dialog, ou launcher dédié) ? | **10** | **ouverte** |
| **CROSS-03** | Le module « Simulation devis » doit-il se lancer sans contexte (choix opportunité dans la modale) quand aucun besoin n'est sélectionné, ou n'apparaître qu'avec contexte ? | **10** | **ouverte** — recommandation : toujours disponible, préset appliqué si contexte |
