...
>
> **Propriétaire :** Guillaume Kasanin · **Version :** 1.3 · **Date :** 2026-07-29 · **Revue :** à chaque nouvelle fiche produite, puis trimestrielle
> **Statut des données citées :** vérifiées en base live le 2026-07-29 (projet Supabase `jvzgmhvwirsbdkjpmvla`), suite à la production de la fiche `transport-mobilite-regionale` — 13 secteurs actifs sur 14, dette #10 fermée (§13).
...
- `metadata.analysis_data` — **93 comptes**. **Structure vérifiée exhaustivement le 2026-07-18 sur les 93 comptes (pas un échantillon) : 100 % homogène.** Toujours exactement 5 clés : `identite`, `positionnement`, `contexte_sectoriel`, `synthese_consultant`, `signaux` — ce dernier objet imbriqué portant `actualites_recentes`, `tendance_croissance`, `recrutements_recents`, `indices_maturite_digitale`. La note précédente (« non uniforme, vérifiée deux fois ») était une fausse alerte — probablement le même piège que celui documenté ci-dessous (chemin JSON supposé sans vérifier, résultat NULL silencieux pris pour une variation de schéma). Vérifie quand même par prudence sur un secteur inconnu, mais ne pars plus du principe que ça varie.
  > ⚠️ **`identite` et `contexte_sectoriel` ne sont pas de la décoration — deux champs concrets à exploiter en Phase 1/Phase 3, absents des requêtes §4.2 telles qu'écrites (elles dumpent le blob entier, donc rien n'est perdu, mais rien n'attire l'œil dessus non plus) :**
  > - **`contexte_sectoriel.concurrents_identifies`** — une seconde liste de concurrents, **distincte** de `sector_analysis.analyse_concurrentielle.concurrents_directs`. Mesuré sur les 76 comptes où les deux listes ont un contenu réel (ni l'une ni l'autre en placeholder) : **overlap moyen 41,5 %, médian 40 %, et 21 % des comptes (16/76) n'ont AUCUN nom en commun** entre les deux listes pour la même entreprise. Ce n'est pas anecdotique (Arkopharma n'est qu'un cas parmi 16) — c'est un pattern structurel : FOLIO a généré deux fois une réponse à « qui sont les concurrents ? » sans les faire converger. **Conséquence pour le bloc Acteurs (§4.3, §6.1) : croiser systématiquement les deux listes.** Un nom présent dans les deux est un signal de confiance (deux passages LLM indépendants s'accordent) ; un nom présent dans une seule est à vérifier avant de l'écrire dans la fiche, pas à recopier tel quel.
  > - **`identite`** (code_naf, ca_estime, effectif_estime, dirigeants) — mesuré : `code_naf` en placeholder (« Non trouvé ») sur **82 %** des comptes, `ca_estime` sur **48 %**, `effectif_estime` sur **66 %**. Ces faits objectifs inférés par LLM+web échouent la majorité du temps — c'est un argument de plus (a posteriori) pour la décision déjà prise ailleurs dans le projet de les chercher via le registre SIRENE officiel plutôt que par inférence. **Ne jamais citer `identite` dans une fiche sans re-vérifier.**
...
| `transport-mobilite-regionale` | **active** | **4.3** | 6 | 7 | 6 | 4 | 0 | **Fiche complète** (2026-07-29) — Gate 3 : 98/100, Axe A 35/35, 7/7 pains traçables, 4/6 échéances datées sourcées (2 volontairement `NULL`, ZFE et NIS2 — cadres instables signalés en caveat plutôt que datés à tort). **Zéro verbatim** (18/18 interactions synthétiques, confirmé). **Zéro ancre de preuve** (le seul `process_diagnostic` du secteur, Cogepart, est la coquille de 62 caractères — 4e occurrence après Horus Pharma/BTP/tourisme) : corpus **moyen**, plafond 4.5
| `ehpad-residences-seniors` | **active** | **4.4** | 2 | 7 | 5 | 4 | 0 | **Fiche complète** (2026-07-26) — Gate 3 : 97/100, Axe A 35/35, 7/7 pains traçables, 4/4 échéances datées sourcées. **Première fiche adossée à un `process_diagnostic` réellement substantiel** (DomusVi, 40 420 car., 11 sections chiffrées) — pas la coquille de 62 car. qui a piégé Horus Pharma/BTP/tourisme. Corpus de 2 comptes seulement, **zéro verbatim** (les 2 interactions sont des e-mails générés par KREDO). ⚠️ Le §3.2 classait ce secteur « corpus mince, plafond 4.0 » **sans avoir lu `content_json`** — plafond réel 4.5 (2 comptes exploitables, pas 0-1) |
...
| `commerce-distribution-services-specialises` | **active** | **3.9** | 12 | ? | ? | ? | 0 | Passé `active` le 2026-07-18 par une session parallèle (migration `20260718170018`) — **non audité**, comptages non vérifiés |
...
|---|---|---|---|---|---|---|---|
| ~~`transport-mobilite-regionale`~~ | 6 | 6/6 | 1 diagnostic (coquille) | **18** | 1 | **4** | ✅ **Vérifié et attaqué le 2026-07-29** — 18/18 interactions synthétiques, 1 mission + 4 opportunités toutes `fictional:true`, diagnostic Cogepart = coquille 62 car. Fiche produite quand même (corpus moyen, 6/6 FOLIO, plafond 4.5, score 4.3). Retiré de cette liste de secteurs `watch` restants. |
...
SELECT sig.signal_type, sig.title, sig.summary, sig.confidence_score,
       sig.relevance_score, sig.urgency_score, sig.detected_at, c.name
-- ⚠️ colonne summary (PAS description, qui n'existe pas — corrigé le 2026-07-26,
--    4e colonne fantôme de ce paquet après expected_close_date et daily_rate_min/max)
...
| `references/schema-supabase.md` et `injection-sql-template.md` **supprimés** | Ils étaient la dérive. Rien ne les remplace : le document les porte. ⚠️ **Réapparus à l'identique et recommittés par erreur le 2026-07-17, re-supprimés le 2026-07-26 — voir §13 #12, ce n'est plus un risque théorique non plus.** |
...
| 10 | **§3.2 comptait des interactions/opportunités de seed comme preuve** | 🔴 Le classement de préparation des secteurs traitait des données `fictional:true`/`synthetic:true` comme des verbatims et un fit réels. A fait passer `sante-medtech-medico-social` puis `tourisme-hotellerie-loisirs` puis `transport-mobilite-regionale` pour « le plus prêt » alors qu'aucun des trois n'avait de verbatim ni de vente réelle — **trois fois consécutives sur ce même tableau**. | ✅ **Corrigé sur les trois secteurs concernés** : `sante-medtech-medico-social` (2026-07-17), `tourisme-hotellerie-loisirs` (2026-07-18), `transport-mobilite-regionale` (2026-07-29 — confirmé : 18/18 interactions synthétiques, mission + 4 opportunités toutes `fictional:true`, rang 🥈 faux). **Dette fermée** : le tableau §3.2 ne compte plus aucun secteur `watch` avec un rang non revérifié. |
...
| 12 | **`references/schema-supabase.md` et `injection-sql-template.md` (supprimés §12.6) réapparus et recommittés sur `main`** | 🔴 Découvert le 2026-07-26 pendant l'étude EHPAD : les deux fichiers étaient de retour sur disque avec **exactement** le contenu cassé du §12.3 (`company_audit`, `companies.ai_score`, `opportunities.status`/`.amount_eur`, template omettant `workspace_id`/`source_company_ids`). Ce n'était pas juste sur disque : `git log --diff-filter=D` montre la suppression dans le commit `ba686091` (2026-07-17 01:00), puis `git show af14410b --stat` montre les deux fichiers réintroduits **tels quels** (214 lignes) dans le commit `af14410b` (« feat: improve opportunity editing and staffing views », 2026-07-17 23:16) — un commit **sans aucun rapport** avec le sectoriel, signe d'un `git add` large qui a ramassé des fichiers reconstitués sur disque entre les deux commits (mécanisme de reconstitution non identifié — probablement historique local d'éditeur ou snapshot, pas un outil de synchro de skill trouvé). Même patron que la dette #2 (`STRATEGIC_SECTOR_CONFIG`), en pire : celle-là avait été rattrapée le jour même, celle-ci est restée committée sur `main` 9 jours. **Corrigé le 2026-07-26** : les deux fichiers remplacés par un stub d'une ligne pointant vers §4.2/§7.1/§7.3 (plus robuste qu'une suppression nue contre une nouvelle réapparition silencieuse — un stub qui se fait écraser laisse un diff visible en `git status`, un fichier supprimé qui réapparaît en untracked peut passer inaperçu dans un `git add -A`). **Non résolu :** `.agents/skills/` est correctement suivi par git (confirmé via `git ls-files` — ce n'est PAS un dossier « ni suivi ni ignoré », seule la vérification via le symlink `.claude/skills/`, sous un `.claude/` globalement gitignoré, donnait ce faux négatif) ; la vraie faille est humaine/procédurale (`git add` large sans relire `git status` avant de committer), pas un trou de configuration git — ne pas ajouter ces chemins au `.gitignore`, ça masquerait la prochaine réapparition au lieu de la révéler. | `git log --oneline --all --diff-filter=D -- '*schema-supabase.md' '*injection-sql-template.md'` ; `git show af14410b --stat` |

| 13 | **`scripts/audit_fiche.py` — faux positif de désaccentuation sur les `source_url`** | 🟠 `walk_strings()` parcourt toutes les chaînes du brouillon, y compris les URLs de sources officielles. Le détecteur de désaccentuation (`DESACCENT_RE`) matchait des marqueurs comme `reglementation`, `securite`, `donnees` **dans le chemin d'une URL** — une URL ne porte jamais d'accent par construction (RFC 3986). Découvert sur `transport-mobilite-regionale` (2026-07-29) : l'URL officielle ANSSI `cyber.gouv.fr/reglementation/...` a fait échouer le Gate 3 alors que 0 champ de texte réel n'était désaccentué. | ✅ **Corrigé** (2026-07-29) : `find_desaccented()` ignore désormais toute chaîne commençant par `http`. **Non-régression vérifiée** sur un cas reconstituant le bug réel documenté §7.2/§10.3 (texte désaccentué + URL officielle dans le même brouillon) : la désaccentuation du texte reste détectée, l'URL n'est plus scannée. |

> **Recommandation :** traiter #1 en priorité — c'est la seule dette d'affichage encore ouverte qui dégrade l'expérience réelle d'un commercial en situation. #4 reste le plus structurant à moyen terme (c'est lui qui bloque le lien automatique secteur → offre → pitch). **#10 fermée** (2026-07-29, trois secteurs vérifiés). **#13 nouvelle et à corriger avant la prochaine étude** si elle cite une URL `.gouv.fr`/`.gov` à risque — sinon le Gate 3 se bloquera sur un faux positif à chaque fois. Auditer encore les 4 fiches de la dette #11 produites hors de cette session (au minimum : Gate 3 rejoué, traçabilité `source_company_ids`, caveats en base). **#12 : vérifier périodiquement (`git log --diff-filter=D -- '*schema-supabase.md' '*injection-sql-template.md'`) que les deux fichiers restent des stubs — une troisième réapparition confirmerait un mécanisme actif à traquer plutôt qu'un accident isolé.**
