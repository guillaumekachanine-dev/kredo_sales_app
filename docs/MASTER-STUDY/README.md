...
## 6. Dette immédiate détectée en établissant ce corpus

Trois constats à traiter, dans cet ordre. Ils ne bloquent pas la lecture du corpus, ils
bloquent son exécution.

1. 🔴 **Les deux skills `.agents/skills/` pointent vers un fichier qui n'existe plus.**
   `kredo-sector-intelligence` déclare `docs/PROCESS-ETUDE-SECTORIELLE.md` comme son
   autorité (frontmatter, corps, `references/`, `scripts/audit_fiche.py`). Le fichier a été
   déplacé en août dans `docs/FEATURES/sector_intelligence/_legacy_kredo_(studies_v1)/`.
   **Un agent qui déclenche ce skill aujourd'hui ne trouve pas sa référence et improvise le
   schéma** — exactement le mode d'échec que le skill dit avoir payé une session entière.
   → Traitement : repointer sur ce corpus (`07-ETAPE-E4…`), pas sur l'archive.
2. 🟠 **`sector_intelligence` : 100 % de la connaissance est sur les 15 macros, 100 % des
   109 comptes sont sur les 38 segments.** 1 seul segment sur 38 porte une fiche remplie.
   Les vues `v_sector_knowledge_*` (migration 069) résolvent la lecture ; **rien n'écrit
   encore à la maille segment**. C'est l'axiome A4 de ce corpus, et le premier invariant que
   G1 vérifie.
3. 🟠 **La Master Study n'a pas encore d'endroit où vivre en base.** `intelligence_documents`
   accepte `primary_entity_type = 'sector'` (enum `intelligence_entity_type`, vérifié live),
   mais l'enum `intelligence_document_type` n'a pas de valeur `master_study`.
   → Une valeur d'enum à ajouter. Aucune table nouvelle. Voir `10-ETAPE-E7…` §5.
